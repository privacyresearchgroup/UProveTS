...
function MsrcryptoMath() {
  ...
  function MontgomeryMultiplier(modulus) {
    ...
    rDigits[s] = 1
    divRem(rDigits, m, quotient, rRemainder, temp1, temp2)
    ...
    divRem(rSquaredDigits, m, quotient, rSquaredModm, temp1, temp2)
    ...
    montgomeryMultiply(rSquaredModm, rSquaredModm, rCubedModm, ctx)
  ...
  function IntegerGroup(modulusBytes) {
    ...
    function createModElementFromBytes(bytes) {
      // NOTE: added for U-Prove
      /// <summary>Create a new element object from a byte value, modulo m_modulus.</summary>
      /// <param name="bytes" type="Array">Desired element in big-endian format in an array of bytes.</param>
      /// <returns type="IntegerGroupElement">An element object representing the given element.</returns>
      ...
      // Check size of the new element
      if (cryptoMath.compareDigits(digits, this.m_modulus) >= 0) {
        // too big; reduce modulo m_modulus
        var remainder = [] // this.createElementFromInteger(1);
        cryptoMath.reduce(digits, m_modulus, remainder, temp0, temp1)
        digits = remainder
      }
    ...
    function IntegerGroupElement(digits, group) {
      ...
      /// digitsToBytes(). Note that other properties and methods are meant
      ...
      return {
        ...
        equals: function (element) {
          ...
          return (
            compareDigits(this.m_digits, element.m_digits) === 0 && this.m_group.equals(this.m_group, element.m_group)
          )
        },

        toByteArrayUnsigned: function () {
          // NOTE: added for U-Prove
          /// <summary>Convert this element to a big-endian byte array.</summary>
          /// <returns>Big-endian byte array of the element.</returns>
          return cryptoMath.digitsToBytes(this.m_digits)
        },
      }
    }
    ...
    return {
      ...
      inverse: inverse,
      modexp: modexp,
      createModElementFromBytes: createModElementFromBytes, // note: added for U-Prove
    }
  }
  ...
  return {
    ...
    gcd: gcd,
    createArray: createArray, // NOTE: added for U-Prove
  }
}

const cryptoMath = MsrcryptoMath()
