import cryptoMath from './msrcrypto/cryptoMath'
import { HashFunctions } from './datatypes'

// tslint:disable: number-literal-format
class SGGroup {
    integerGroup: any
    constructor(integerGroup) {
        this.integerGroup = integerGroup
    }

    // allocates an element to store some computation results
    getIdentityElement(): any {
        // return the point at infinity
        return this.integerGroup.createElementFromInteger(1)
    }

    // creates an element from the serialized bytes
    createElementFromBytes(bytes): any {
        return this.integerGroup.createElementFromBytes(bytes)
    }

    modexp(base, exp, result): any {
        return this.integerGroup.modexp(base, exp.m_digits, result)
    }

    // computes result = a + b
    multiply(a, b, result): any {
        return this.integerGroup.multiply(a, b, result)
    }
}

class L2048N256Object {
    // recommended parameters

    p = Uint8Array.from([
        ,
    ])
    q = Uint8Array.from([
        0xc8,
        0xf7,
        0x50,
        0x94,
        0x1d,
        0x91,
        0x79,
        0x19,
        0x04,
        0xc7,
        0x18,
        0x6d,
        0x62,
        0x36,
        0x8e,
        0xc1,
        0x9e,
        0x56,
        0xb3,
        0x30,
        0xb6,
        0x69,
        0xd0,
        0x87,
        0x08,
        0xf8,
        0x82,
        0xe4,
        0xed,
        0xb8,
        0x28,
        0x85,
    ])
    g = Uint8Array.from([
        ,
    ])

    // number of pregenerated generators
    n = 50
    // gt index
    t = this.n + 1

    // the pregenerated parameters g1, ..., gn, gt
    generators = [
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
        Uint8Array.from([
            ,
        ]),
    ]

    e = Uint8Array.from([
        ,
    ])

    Gq = new SGGroup(cryptoMath.IntegerGroup(this.p))
    getGq = () => this.Gq

    Zq = cryptoMath.IntegerGroup(this.q)
    getZq = () => this.Zq

    getGenerator = () => this.Gq.createElementFromBytes(this.g)

    // update the hash with the group values
    // hash   - UProve.Hash          - the hash function to update
    updateHash = (hash: HashFunctions) => {
        hash.updateBytes(this.p)
        hash.updateBytes(this.q)
        hash.updateBytes(this.g)
    }

    // returns an array of n + 2 pre-generated generators: 1, g1, ..., gn, gt.
    // The first element (g0) is set to 1 and must be replaced by caller with
    // an Issuer-specific value.
    getPreGenGenerators = (n: number) => {
        const gen = new Array(n + 2)
        gen[0] = this.Gq.getIdentityElement() // to be replaced by caller
        for (let i = 1; i <= n; i++) {
            // g1, ..., gn
            gen[i] = this.Gq.createElementFromBytes(this.generators[i - 1])
        }
        gen[n + 1] = this.Gq.createElementFromBytes(this.generators[this.t - 1])
        return gen
    }

    eElement = this.Gq.createElementFromBytes(this.e)
    // generateScopeElement = (s: Uint8Array) => {
    //     if (!s) {
    //         throw new Error('invalid scope')
    //     }

    //     const index = 0
    //     let count = 0
    //     const g = this.Gq.createElementFromInteger(0)

    //     let hashInput = Array.from(s) // Array.apply([], s)
    //     hashInput = hashInput.concat([0x67, 0x67, 0x65, 0x6e, index, count])
    //     while (cryptoMath.compareDigits(g.m_digits, this.Gq.m_two) < 0) {
    //         // g < 2
    //         if (count === 255) {
    //             throw new Error("can't generate scope element, max count has been reached")
    //         }
    //         count++
    //         const hash = new Hash()
    //         hashInput[hashInput.length - 1] = count
    //         hash.updateRawArray(hashInput)
    //         const W = this.Gq.createElementFromBytes(hash.digest())
    //         this.Gq.modexp(W, this.eElement, g)
    //     }
    //     return g
    // }
    OID = '1.3.6.1.4.1.311.75.1.1.1'
}

const L2048N256 = new L2048N256Object()
export default L2048N256
